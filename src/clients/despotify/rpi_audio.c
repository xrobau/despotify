...
void audio_play_pcm(void* device, struct ds_pcm_data* pcm)
{
   AUDIOPLAY_STATE_T *st = device;

   uint8_t *buf;
   uint32_t latency, ret;
   unsigned int samplerate = 44100;

   while((buf = audioplay_get_buffer(st)) == NULL)
      usleep(10*1000);

   memcpy(buf, pcm->buf, pcm->len);

   while((latency = audioplay_get_latency(st)) > (samplerate * (MIN_LATENCY_TIME + CTTW_SLEEP_TIME) / 1000))
      usleep(CTTW_SLEEP_TIME*1000);

   ret = audioplay_play_buffer(st, buf, pcm->len);
   assert(ret == 0);
}

void audio_exit(void* device)
{
   AUDIOPLAY_STATE_T *st = device;
   audioplay_delete(st);   
}
